...
@endif
<a href="{{ route('student', ['id' => auth()->user()->id]) }}">
...
<div id="profileAvatar"
    ...
    <img id="previewImage"
         src="{{ $students->image ? asset('storage/' . $students->image) : asset('images/default.png') }}"
         alt="Profile Image"
...
<input type="file" name="image" id="imageInput" style="display:none;">
 ...
 <div class="col-md-9">
                    <label for="University" class="form-label small mb-1">الجامعة</label>
                    <input type="text" name="University" id="last_name"
                           class="form-control form-control-sm mb-2"
                           value="{{ old('University', auth()->user()->student->University) }}">
                </div>

 <div class="col-md-9">
                    <label for="University_Major" class="form-label small mb-1">الجامعة</label>
                    <input type="text" name="University_Major" id="last_name"
                           class="form-control form-control-sm mb-2"
                           value="{{ old('University_Major', auth()->user()->student->University_Major) }}">
                </div>
                   <div class="col-md-9">
                    <label for="location" class="form-label small mb-1">اهتماماتك التقنية:</label>
                    <select class="form-select form-select-sm mb-2" id="location" name="technica_interests" required>
                        @php
                            $selectedtechnica_interests = old('technica_interests', auth()->user()->student->technica_interests);
                        @endphp
                        <option value="web-development" {{ $selectedtechnica_interests == 'web-development' ? 'selected' : '' }}>       تطوير الويب</option>
                        <option value="mobile-development" {{ $selectedtechnica_interests == 'mobile-development' ? 'selected' : '' }}> تطوير التطبيقات</option>
                        <option value="ui-ux-design" {{ $selectedtechnica_interests == 'ui-ux-design' ? 'selected' : '' }}>تصميم واجهات وتجربة المستخدم</option>
  <option value="data-science" {{ $selectedtechnica_interests == 'data-science' ? 'selected' : '' }}> علوم البيانات</option>
                        <option value="cybersecurity" {{ $selectedtechnica_interests == 'cybersecurity' ? 'selected' : '' }}>الأمن السيبراني</option>
                        <option value="cloud-computing" {{ $selectedtechnica_interests == 'cloud-computing' ? 'selected' : '' }}>الحوسبة السحابية</option>
                        <option value="digital-marketing" {{ $selectedtechnica_interests == 'digital-marketing' ? 'selected' : '' }}> التسويق الرقمي</option>
                        <option value="game-development" {{ $selectedtechnica_interests == 'game-development' ? 'selected' : '' }}>    تطوير الألعاب</option>
                                              </select>
                </div>
                  <div class="col-md-9">
                    <label for="location" class="form-label small mb-1">السنة الدراسية</label>
                    <select class="form-select form-select-sm mb-2" id="location" name="Acabemic_Year" required>
                        @php
                            $selectedAcabemic_Year = old('Acabemic_Year', auth()->user()->student->Acabemic_Year);
                        @endphp
                        <option value="1" {{ $selectedAcabemic_Year == '1' ? 'selected' : '' }}>       السنة الأولى</option>
                        <option value="2" {{ $selectedAcabemic_Year == '2' ? 'selected' : '' }}>السنة الثانية</option>
                        <option value="3" {{ $selectedAcabemic_Year == '3' ? 'selected' : '' }}>السنة الثالثة</option>
  <option value="4" {{ $selectedAcabemic_Year == '4' ? 'selected' : '' }}> السنة الرابعة</option>
                        <option value="5" {{ $selectedAcabemic_Year == '5' ? 'selected' : '' }}>  السنة الخامسة </option>
                        <option value="graduate" {{ $selectedAcabemic_Year == 'graduate' ? 'selected' : '' }}> دراسات عليا</option>
                                              </select>
                </div>
                <div class="col-md-9">
 ...
 <label>السيرة الذاتية (PDF, Word):</label>
    <input type="file" name="cv" accept=".pdf,.doc,.docx">
          ...
          <textarea name="bio" rows="6" class="form-control">{{ old('bio', auth()->user()->student?->bio) }}</textarea>
        </div>
    <div class="content-card mb-4">
    ...
    @php
        ...
        $projects = $oldProjects ?? ( $students->students ?  $students->students->projects : []);
    @endphp
